...
CREATE TABLE `ledger` (
  ...
  `book_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '采纳到book里的卡牌组合的id，统一走治理路线，一个组合就存一个组合，要存多个，走key_json，声明这是引用自哪条',
  `player_remark_json` json NOT NULL DEFAULT (json_object()),
  `remark_json` json NOT NULL DEFAULT (json_object()) COMMENT '系统治理者的备注字段',
  `status_id` int unsigned NOT NULL DEFAULT '1',
  `created_at` datetime NOT NULL DEFAULT (now()),
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI训练审计账本';
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2026-02-18 22:48:12
